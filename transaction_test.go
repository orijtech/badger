...
func TestTxnSimple(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		item, err := txn.Get(ctx, []byte("key=8"))
		...
		require.Error(t, ErrManagedTxn, txn.CommitAt(ctx, 100, nil))
	...
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		for i := 0; i < N; i++ {
			go func(i int) {
				...
				err := db.Update(ctx, func(ctx context.Context, tx *Txn) error {
					return tx.Set(ctx, key, key)
				})
				require.NoError(t, err)
				err = db.View(ctx, func(ctx context.Context, tx *Txn) error {
					item, err := tx.Get(ctx, key)
					require.NoError(t, err)
					val, err := item.Value()
					require.NoError(t, err)
...
func TestTxnCommitAsync(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		for i := 0; i < 40; i++ {
			err := txn.Set(ctx, key(i), []byte(strconv.Itoa(100)))
			require.NoError(t, err)
		}
		require.NoError(t, txn.Commit(ctx, nil))
		txn.Discard(ctx)
		...
		for i := 0; i < 100; i++ {
			go func() {
				...
				delta := rand.Intn(100)
				for i := 0; i < 20; i++ {
					err := txn.Set(ctx, key(i), []byte(strconv.Itoa(100-delta)))
					require.NoError(t, err)
				}
				for i := 20; i < 40; i++ {
					err := txn.Set(ctx, key(i), []byte(strconv.Itoa(100+delta)))
				...
				require.NoError(t, txn.Commit(ctx, func(err error) {}))
				txn.Discard(ctx)
				wg.Done()
			}()
		}
...
func TestTxnVersions(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		for i := 1; i < 10; i++ {
			...
			txn.Set(ctx, k, []byte(fmt.Sprintf("valversion=%d", i)))
			require.NoError(t, txn.Commit(ctx, nil))
...
func TestTxnWriteSkew(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		txn1 := db.NewTransaction(ctx, true)

		sum := getBal(txn1, ax)
		sum += getBal(txn1, ay)
		require.Equal(t, 200, sum)
		txn1.Set(ctx, ax, []byte("0")) // Deduct 100 from ax.
		...
		sum += getBal(txn2, ay)
		require.Equal(t, 200, sum)
		txn2.Set(ctx, ay, []byte("0")) // Deduct 100 from ay.
		...
		// Commit both now.
		require.NoError(t, txn1.Commit(ctx, nil))
		require.Error(t, txn2.Commit(ctx, nil)) // This should fail.
...
func TestTxnIterationEdgeCase(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		// c1
		txn := db.NewTransaction(ctx, true)
		txn.Set(ctx, kc, []byte("c1"))
		require.NoError(t, txn.Commit(ctx, nil))
		require.Equal(t, uint64(1), db.orc.readTs())

		// a2, c2
		txn = db.NewTransaction(ctx, true)
		txn.Set(ctx, ka, []byte("a2"))
		txn.Set(ctx, kc, []byte("c2"))
		require.NoError(t, txn.Commit(ctx, nil))
		require.Equal(t, uint64(2), db.orc.readTs())

		// b3
		txn = db.NewTransaction(ctx, true)
		txn.Set(ctx, ka, []byte("a3"))
		txn.Set(ctx, kb, []byte("b3"))
		require.NoError(t, txn.Commit(ctx, nil))
		require.Equal(t, uint64(3), db.orc.readTs())

		// b4, c4(del) (Uncomitted)
		txn4 := db.NewTransaction(ctx, true)
		require.NoError(t, txn4.Set(ctx, kb, []byte("b4")))
		require.NoError(t, txn4.Delete(ctx, kc))
		require.Equal(t, uint64(3), db.orc.readTs())
		...
		defer txn.Discard(ctx)
		itr := txn.NewIterator(ctx, DefaultIteratorOptions)
		itr5 := txn4.NewIterator(ctx, DefaultIteratorOptions)
		checkIterator(itr, []string{"a3", "c2"})
		checkIterator(itr5, []string{"a3", "b4"})

		rev := DefaultIteratorOptions
		rev.Reverse = true
		itr = txn.NewIterator(ctx, rev)
		itr5 = txn4.NewIterator(ctx, rev)
...
func TestTxnIterationEdgeCase3(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		// b2
		txn = db.NewTransaction(ctx, true)
		txn.Set(ctx, kb, []byte("b2"))
		require.NoError(t, txn.Commit(ctx, nil))
		require.Equal(t, uint64(2), db.orc.readTs())

		txn2 := db.NewTransaction(ctx, true)
		require.NoError(t, txn2.Set(ctx, kd, []byte("d2")))
		require.NoError(t, txn2.Delete(ctx, kc))
		...
		//  Keys: "abc", "ade"
		// Read pending writes.
		itr = txn2.NewIterator(ctx, DefaultIteratorOptions)
		itr.Seek([]byte("ab"))
		require.True(t, itr.Valid())
		require.Equal(t, itr.item.Key(), kb)
		itr.Seek([]byte("ac"))
...
func TestIteratorAllVersionsWithDeleted(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		err := db.Update(ctx, func(ctx context.Context, txn *Txn) error {
			txn.Set(ctx, []byte("answer1"), []byte("42"))
			txn.Set(ctx, []byte("answer2"), []byte("43"))
			return nil
		})
		require.NoError(t, err)
		...
		err = db.View(ctx, func(ctx context.Context, txn *Txn) error {
			item, err := txn.Get(ctx, []byte("answer1"))
			require.NoError(t, err)
			err = txn.db.batchSet(ctx, []*Entry{
				{
...
func TestIteratorAllVersionsWithDeleted2(t *testing.T) {
	runBadgerTest(context.Background(), t, nil, func(ctx context.Context, t *testing.T, db *DB) {
		...
		for i := 0; i < 4; i++ {
			err := db.Update(ctx, func(ctx context.Context, txn *Txn) error {
				if i%2 == 0 {
					txn.Set(ctx, []byte("key"), []byte("value"))
					return nil
				}
				txn.Delete(ctx, []byte("key"))
				return nil
			})
			require.NoError(t, err)
		}
...
func TestManagedDB(t *testing.T) {
	...
	err = kv.Update(ctx, func(ctx context.Context, tx *Txn) error { return nil })
	require.Equal(t, ErrManagedTxn, err)

	err = kv.View(ctx, func(ctx context.Context, tx *Txn) error { return nil })
	require.Equal(t, ErrManagedTxn, err)

	// Write data at t=3.
	txn := kv.NewTransactionAt(ctx, 3, true)
	for i := 0; i <= 3; i++ {
		require.NoError(t, txn.Set(ctx, key(i), val(i)))
	}
	require.Error(t, ErrManagedTxn, txn.Commit(ctx, nil))
	require.NoError(t, txn.CommitAt(ctx, 3, nil))

	// Read data at t=2.
	txn = kv.NewTransactionAt(ctx, 2, false)
	for i := 0; i <= 3; i++ {
		_, err := txn.Get(ctx, key(i))
		require.Equal(t, ErrKeyNotFound, err)
	}
	txn.Discard(ctx)

	// Read data at t=3.
	txn = kv.NewTransactionAt(ctx, 3, false)
	for i := 0; i <= 3; i++ {
		item, err := txn.Get(ctx, key(i))
		require.NoError(t, err)
	...
	// Write data at t=7.
	txn = kv.NewTransactionAt(ctx, 6, true)
	for i := 0; i <= 7; i++ {
		_, err := txn.Get(ctx, key(i))
		if err == nil {
	...
	require.NoError(t, txn.CommitAt(ctx, 7, nil))

	// Read data at t=9.
	txn = kv.NewTransactionAt(ctx, 9, false)
	for i := 0; i <= 9; i++ {
		item, err := txn.Get(ctx, key(i))
		if i <= 7 {
			require.NoError(t, err)
...
func TestArmV7Issue311Fix(t *testing.T) {
	...
	err = db.View(ctx, func(ctx context.Context, txn *Txn) error { return nil })
	if err != nil {
		t.Fatal(err)
	}

	err = db.Update(ctx, func(ctx context.Context, txn *Txn) error {
		return txn.Set(ctx, []byte{0x11}, []byte{0x22})
	})
	if err != nil {
		t.Fatal(err)
	}
